...
def check_favorites_request_type(request, user, recipe):
    ...
    if request.method == 'POST':
        if api_models.Favorites.objects.filter(user=user, recipe=recipe).exists():
            raise api_exception.FavouriteException(
                {'errors': f'Невозможно добавить {recipe.name}, так как он находится в избранном'},
            )
    ...
    if request.method == 'DELETE':
        if api_models.Favorites.objects.filter(user=user, recipe=recipe).exists():
            api_models.Favorites.objects.filter(user=user, recipe=recipe).delete()
            return "delete"
...
def check_list_shopping_request_type(request, user, recipe, pk):
    if request.method == 'POST':
        if api_models.ListShopping.objects.filter(user=user, recipe=recipe).exists():
    ...
    if request.method == 'DELETE':
        if api_models.ListShopping.objects.filter(user=user, recipe__id=pk).exists():
            api_models.ListShopping.objects.filter(user=user, recipe__id=pk).delete()
            return "delete"
        return api_exception.ListShopingException(
